...
function table_name (model_name) {
  str = inflection.pluralize(model_name);
  str = str.toLowerCase();
  return str;
}
...
/**
 * Load is like a GET except we have to build the where / join
 * clause to make sure we retrieve childs that are related to parent.
 *
 * TODO: THIS SHOULD NOT BE IMPLEMENTED AT ADAPTER LEVEL!!!
 * should be transfomed to get().where('parent_property': parent)
 *
 */
SQL.prototype.load = function (query, model, parent_obj, cb) {
  var rel = model.relationships[query.load],
    foreignModel = rel.model,
    foreign_id, where, include;
  ...
  if (rel.type == 'hasOne') {
    ...
    foreignModel.id().forEach(function (attr) {
      // heuristic to guess the name of the foreign key
      // property_name_idattr
      where[attr] = parent_obj[rel.name.toLowerCase() + '_' + attr];
    });
    query.where(where);
    model = foreignModel;

    this.select(query, model, null, cb);
  }
  else if (rel.type == 'hasMany') {
    /*
     * @TODO I think this might overwrite unsaved modifications to parent object :(
     */
    where = {};
    model.id().forEach(function (attr) {
      where[attr] = parent_obj[attr];
    });

    query.where(where);
    query.expr.include.push(query.load);

    this.select(query, model, null, function (err, res) {
      // only return what we needed to load, not the parent objects
      if (err) return cb(err);
      res.forEach(function (obj, i) {
        res[i] = obj[query.load];
      });
...
SQL.prototype.select = function (query, model, values, cb) {
  ...
  if (query.expr.include) {
    ...
    includes.forEach(function (rel_name) {
      ...
      if (!rel) {
        throw new Error(model.name + ': Attempting to include ' + rel_name + ' but ' + rel_name + ' is not a relationship.');
      }
